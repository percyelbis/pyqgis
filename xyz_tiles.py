...
sources.append(["connections-xyz","Google Roads", "", "", "", "https://mt1.google.com/vt/lyrs=r&x=%7Bx%7D&y=%7By%7D&z=%7Bz%7D", "", "20", "0"])
sources.append(["connections-xyz","Google Labels", "", "", "", "https://mt1.google.com/vt/lyrs=h&x=%7Bx%7D&y=%7By%7D&z=%7Bz%7D", "", "20", "0"])
sources.append(["connections-xyz","Esri World Imagery Clarity", "", "", "", "https://clarity.maptiles.arcgis.com/arcgis/rest/services/World_Imagery/MapServer/tile/%7Bz%7D/%7By%7D/%7Bx%7D", "", "20", "0"])
sources.append(["connections-xyz","CartoDb Voyager", "", "", "", "https://basemaps.cartocdn.com/rastertiles/voyager/%7Bz%7D/%7Bx%7D/%7By%7D.png", "", "20", "0"])
sources.append(["connections-xyz","CyclOSM", "", "", "", "https://a.tile-cyclosm.openstreetmap.fr/cyclosm/%7Bz%7D/%7Bx%7D/%7By%7D.png", "", "19", "0"])
sources.append(["connections-xyz","Sentinel-2 Cloudless (EOX)", "", "", "", "https://tiles.maps.eox.at/wmts/1.0.0/s2cloudless-2020_3857/default/GoogleMapsCompatible/%7Bz%7D/%7By%7D/%7Bx%7D.jpg", "", "14", "0"])

# Guardar cada fuente usando QgsSettings (compatible con QGIS 3.x)
s = QgsSettings()
for source in sources:
    connectionType = source[0]
    connectionName = source[1]
    path_url = "qgis/%s/%s/url" % (connectionType, connectionName)
    
    # Prevención de duplicados
    if s.contains(path_url):
        existing_url = s.value(path_url)
        if existing_url == source[5]:
            print("✔ Ya existe (idéntico):", connectionName)
            continue
        else:
            print("🔄 Actualizando:", connectionName)
    else:
        print("➕ Agregado:", connectionName)

    s.setValue("qgis/%s/%s/authcfg" % (connectionType, connectionName), source[2])
    s.setValue("qgis/%s/%s/password" % (connectionType, connectionName), source[3])
    s.setValue("qgis/%s/%s/referer" % (connectionType, connectionName), source[4])
    s.setValue("qgis/%s/%s/url" % (connectionType, connectionName), source[5])
    s.setValue("qgis/%s/%s/username" % (connectionType, connectionName), source[6])
    s.setValue("qgis/%s/%s/zmax" % (connectionType, connectionName), source[7])
    s.setValue("qgis/%s/%s/zmin" % (connectionType, connectionName), source[8])

# Recargar conexiones en la interfaz
iface.reloadConnections()
print("✅ ¡Listo! Revisa el Panel Navegador → XYZ Tiles")
